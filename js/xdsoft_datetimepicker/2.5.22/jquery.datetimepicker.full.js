...
/*
 * 2.5.22
 * added method refreshTimePicker to detect disabled time by ajax request
 */

/**
 * @param {jQuery} $
 */
var xdatetimepickerFactory = function ($) {
	'use strict';
	...
	$.extend($.xdatetimepicker, standardFormats);
				...
				timeboxparent
					.on('scroll_element.xdsoft_scroller', function (event, percentage) {
						...
						timeboxTop = parseFloat(Math.abs((timebox[0].offsetHeight - parentHeight) * percentage).toFixed(4));
                                                ...
                                                //console.log(timebox[0].offsetHeight)
                                                
                                                if(timebox[0].offsetHeight<=175)
                                                {
                                                    timeboxTop = 0;
                                                }                                                
	...
	$.fn.xdatetimepicker = function (opt, opt2) {
		var result = this,
			KEY0 = 48,
			KEY9 = 57,
			_KEY0 = 96,
		...
		createDateTimePicker = function (input) {
			...
			var xdatetimepicker = $('<div class="xdsoft_datetimepicker xdsoft_noselect"></div>'),
				...
				applyButton = $('<button type="button" class="xdsoft_save_selected blue-gradient-button">Save Selected</button>'),
                                clearButton = $('<div class="xdsoft_label xdsoft_clear">'+i18n['TEXT_CLEAR']+'</div>'),                                                                
			...
			if (options.id) {
				xdatetimepicker.attr('id', options.id);
			}
			if (options.style) {
				xdatetimepicker.attr('style', options.style);
			}
			if (options.weeks) {
				xdatetimepicker.addClass('xdsoft_showweeks');
			}
			if (options.rtl) {
				xdatetimepicker.addClass('xdsoft_rtl');
			}

			xdatetimepicker.addClass('xdsoft_' + options.theme);
			xdatetimepicker.addClass(options.className);
			...
			month_picker
				...
				.on('touchend mousedown.xdsoft', '.xdsoft_option', function () {
					if (!this.touchMoved) {
						...
						xdatetimepicker.trigger('xchange.xdsoft');
						if (options.onChangeMonth && typeof options.onChangeMonth === 'function') {
							options.onChangeMonth.call(xdatetimepicker, _xdsoft_datetime.currentTime, xdatetimepicker.data('input'));
						}

						if (year !== _xdsoft_datetime.currentTime.getFullYear() && typeof options.onChangeYear === 'function') {
							options.onChangeYear.call(xdatetimepicker, _xdsoft_datetime.currentTime, xdatetimepicker.data('input'));
						}
					}
				});

			xdatetimepicker.getValue = function () {
				return _xdsoft_datetime.getCurrentTime();
			};

			xdatetimepicker.setOptions = function (_options) {
				...
				if (options.inline) {
					...
					xdatetimepicker.addClass('xdsoft_inline');
					input.after(xdatetimepicker).hide();
				}
					...
					input
						...
						.on('blur.xdsoft', function () {
							if (options.allowBlank && (!$(this).val().trim().length ||
								...
								$(this).val(null);
								xdatetimepicker.data('xdsoft_datetime').empty();
							} else {
								...
								xdatetimepicker.data('xdsoft_datetime').setCurrentTime($(this).val());
							}

							xdatetimepicker.trigger('changedatetime.xdsoft');
							xdatetimepicker.trigger('close.xdsoft');
						});
				}
			...
			xdatetimepicker.on('afterOpen.xdsoft', function () {
				timeboxparent.xdsoftScroller(options);
			});
            ...
            if (options.insideParent) {
                $(input).parent().append(xdatetimepicker);
            } else {
                $(options.parentID).append(xdatetimepicker);
            }
			...
			XDSoft_datetime = function () {
				...
				_this.getWeekOfYear = function (datetime) {
					if (options.onGetWeekOfYear && typeof options.onGetWeekOfYear === 'function') {
						var week = options.onGetWeekOfYear.call(xdatetimepicker, datetime);
			...
			applyButton.on('touchend click', function (e) {//pathbrite
				e.preventDefault();
				xdatetimepicker.data('changed', true);
				_xdsoft_datetime.setCurrentTime(getCurrentValue());
				input.val(_xdsoft_datetime.str());
				xdatetimepicker.trigger('close.xdsoft');
			});
                        
                        clearButton.on('touchend click', function (e) {//pathbrite
				e.preventDefault();                                
			...
			month_picker
				...
				.on('touchend mousedown.xdsoft', function () {
					...
					xdatetimepicker.trigger('afterOpen.xdsoft');
					...
					xchangeTimer = setTimeout(function () {
						...
						while (i < _xdsoft_datetime.currentTime.countDaysInMonth() || start.getDay() !== options.dayOfWeekStart || _xdsoft_datetime.currentTime.getMonth() === start.getMonth()) {
							...
							if (options.beforeShowDay && typeof options.beforeShowDay.call === 'function') {
								customDateSettings = options.beforeShowDay.call(xdatetimepicker, start);
							} else {
							...
							if ((options.defaultSelect || xdatetimepicker.data('changed')) && dateHelper.formatDate(_xdsoft_datetime.currentTime, options.formatDate) === dateHelper.formatDate(start, options.formatDate)) {
						...
						line_time = function line_time(h, m) {
							...
							if ((options.initTime || options.defaultSelect || xdatetimepicker.data('changed')) && current_time.getHours() === parseInt(h, 10) && ((!isALlowTimesInit && options.step > 59) || current_time.getMinutes() === parseInt(m, 10))) {
								if (options.defaultSelect || xdatetimepicker.data('changed')) {
									classes.push('xdsoft_current');
							...
							time += '<div class="xdsoft_time ' + classes.join(' ') + '" data-hour="' + h + '" data-minute="' + m + '" data-hm="'+h+':'+m+'">' + dateHelper.formatDate(now, options.formatTime) + '</div>';
                                                        
                                                        options.allowTimesHours.push(h+':'+m)
						};


                                                //reset allowTimesHours
                                                options.allowTimesHours = []
			...
			calendar
				.on('touchend click.xdsoft', 'td', function (xdevent) {
					...
					currentTime.setDate($this.data('date'));
                                        ...
                                        if(options.allowTimesHours.length>0 && !input.hasClass('xdsoftDateSelected'))
                                        {
                                            //console.log(options.allowTimesHours)

                                            var is_valid_time = false

                                            options.allowTimesHours.forEach((v,k)=>{
                                                if(v.split(':')[0]==currentTime.getHours())
                                                {
                                                    is_valid_time = true;
                                                    
                                                    currentTime.setHours(v.split(':')[0])
                                                    currentTime.setMinutes(v.split(':')[1])
                                                }
                                            })

                                            if(!is_valid_time)
                                            {
                                                currentTime.setHours(options.allowTimesHours[0].split(':')[0])
                                                currentTime.setMinutes(options.allowTimesHours[0].split(':')[1])
                                            }
                                            
                                            input.addClass('xdsoftDateSelected')
                                        }
                                        
                                        
                                        
                                        xdatetimepicker.trigger('select.xdsoft', [currentTime]);
                                        

                                        //set input val when time is selected only
                                        if(options.disabledTime.length==0)
                                        {
                                            input.val(_xdsoft_datetime.str());
                                        }

					if (options.onSelectDate &&	typeof options.onSelectDate === 'function') {
						options.onSelectDate.call(xdatetimepicker, _xdsoft_datetime.currentTime, xdatetimepicker.data('input'), xdevent);
					}
                                        
                                        refreshTimePicker(_xdsoft_datetime.currentTime,timebox)
					...
					if (!this.touchMoved) {
						...
						if ($this.hasClass('xdsoft_disabled')) {
							return false;
                                                ...
                                                if(options.allowTimesHours.length>0)
                                                {
                                                   if(!options.allowTimesHours.includes($this.data('hour')+':'+$this.data('minute'))) 
                                                   {
                                                       return false
                                                   }
                                                }
                                                //console.log($this.data('hour')+':'+$this.data('minute'))
                                                
						currentTime.setHours($this.data('hour'));
						currentTime.setMinutes($this.data('minute'));
						xdatetimepicker.trigger('select.xdsoft', [currentTime]);

						xdatetimepicker.data('input').val(_xdsoft_datetime.str());

						if (options.onSelectTime && typeof options.onSelectTime === 'function') {
							options.onSelectTime.call(xdatetimepicker, _xdsoft_datetime.currentTime, xdatetimepicker.data('input'), xdevent);
						}
						xdatetimepicker.data('changed', true);
						xdatetimepicker.trigger('xchange.xdsoft');
						xdatetimepicker.trigger('changedatetime.xdsoft');
			...
			xdatetimepicker
				.on('changedatetime.xdsoft', function (event) {
					if (options.onChangeDateTime && typeof options.onChangeDateTime === 'function') {
						var $input = xdatetimepicker.data('input');
						options.onChangeDateTime.call(xdatetimepicker, _xdsoft_datetime.currentTime, $input, event);
				...
				.on('generate.xdsoft', function () {
					if (options.onGenerate && typeof options.onGenerate === 'function') {
						options.onGenerate.call(xdatetimepicker, _xdsoft_datetime.currentTime, xdatetimepicker.data('input'));
					}
					if (triggerAfterOpen) {
						xdatetimepicker.trigger('afterOpen.xdsoft');
						triggerAfterOpen = false;
			...
			setPos = function () {
				...
				$dateInput = xdatetimepicker.data('input');
				...
				if ($dateInput.parent().css('direction') === 'rtl') {
					left -= (xdatetimepicker.outerWidth() - $dateInput.outerWidth());
				}
				...
				if (options.fixed) {
					...
					if (dateInputHasFixedAncestor && !options.insideParent) {
						...
						if (verticalPosition + xdatetimepicker.outerHeight() > windowHeight + windowScrollTop) {
						...
						if (verticalPosition + xdatetimepicker[0].offsetHeight > windowHeight + windowScrollTop) {
							verticalPosition = dateInputOffset.top - xdatetimepicker[0].offsetHeight + 1;
						}
					}
				...
				datetimepickerElem = xdatetimepicker[0];
				...
				xdatetimepicker.css(datetimepickerCss);
			};
			...
			xdatetimepicker
				.on('open.xdsoft', function (event) {
					...
					if (options.onShow && typeof options.onShow === 'function') {
						onShow = options.onShow.call(xdatetimepicker, _xdsoft_datetime.currentTime, xdatetimepicker.data('input'), event);
					}
                                        
                                        refreshTimePicker(_xdsoft_datetime.currentTime,timebox)
                                        
					if (onShow !== false) {
						xdatetimepicker.show();
						setPos();
				...
				.on('close.xdsoft', function (event) {
					...
					if (options.onClose && typeof options.onClose === 'function') {
						onClose = options.onClose.call(xdatetimepicker, _xdsoft_datetime.currentTime, xdatetimepicker.data('input'), event);
					}
					if (onClose !== false && !options.opened && !options.inline) {
						xdatetimepicker.hide();
					}
					event.stopPropagation();
				})
				.on('toggle.xdsoft', function () {
					if (xdatetimepicker.is(':visible')) {
						xdatetimepicker.trigger('close.xdsoft');
					} else {
						xdatetimepicker.trigger('open.xdsoft');
					}
				})
				.data('input', input);
			...
			xdatetimepicker.data('xdsoft_datetime', _xdsoft_datetime);
			xdatetimepicker.setOptions(options);
			...
			input
				.data('xdsoft_datetimepicker', xdatetimepicker)
                ...
                //if disabledTime setup then refersh timebox and set disabled time
                refreshTimePicker = function(ct,timebox){
                    if(options.disabledTime.length==0)
                    {
                        return false
                    }
                    
                    let url = options.disabledTime
                   
                    let m = ct.getMonth() + 1; //Month from 0 to 11
                    let d = ct.getDate();
                    let date = ct.getFullYear()+'-'+(m<=9 ? '0' + m : m) + '-' + (d <= 9 ? '0' + d : d) 
                                        
                    $.ajax({url:url+'&date='+date}).done(function(response, status, xhr) {                         
                       if(is_json(response))
                       {
                           //$('.xdsoft_time_variant .xdsoft_time').addClass('hidden')
                           let result = JSON.parse(response) 

                           for(let key in result)
                           {
                                let time = result[key]  
                                const index = options.allowTimesHours.indexOf(time)
                                if (index > -1) { // only splice array when item is found
                                    options.allowTimesHours.splice(index, 1); // 2nd parameter means remove one item only                                                                        
                                    timebox.find('[data-hm="'+time+'"]').addClass('xdsoft_disabled')                                    
                                }
                           }   
                       ...
                       {
                           console.error(strip_tags(response))
                       }
                    })
                   
                };
		destroyDateTimePicker = function (input) {
			var xdatetimepicker = input.data('xdsoft_datetimepicker');
			if (xdatetimepicker) {
				xdatetimepicker.data('xdsoft_datetime', null);
				xdatetimepicker.remove();
				input
		...
		this.each(function () {
			var xdatetimepicker = $(this).data('xdsoft_datetimepicker'), $input;
			if (xdatetimepicker) {
				if (typeof opt === 'string') {
					switch (opt) {
						...
						case 'hide':
							...
							break;
						case 'toggle':
							xdatetimepicker.trigger('toggle.xdsoft');
							break;
						case 'destroy':
						...
						case 'reset':
							...
							if (!this.value || !xdatetimepicker.data('xdsoft_datetime').isValidDate(dateHelper.parseDate(this.value, options.format))) {
								xdatetimepicker.data('changed', false);
							}
							xdatetimepicker.data('xdsoft_datetime').setCurrentTime(this.value);
							break;
						case 'validate':
							$input = xdatetimepicker.data('input');
							$input.trigger('blur.xdsoft');
							break;
						default:
							if (xdatetimepicker[opt] && typeof xdatetimepicker[opt] === 'function') {
								result = xdatetimepicker[opt](opt2);
							}
					}
				} else {
					xdatetimepicker
	...
	$.fn.xdatetimepicker.defaults = default_options;
