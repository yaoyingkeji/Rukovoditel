    ...
    if (app_parse_search_string($_POST['search_keywords'], $search_keywords))
    {
        ...
        {                        
            ...
            elseif ($field['type'] == 'fieldtype_tags')
            {
                ...
                {
                    $sql_query[] = "(select count(*) as total from app_entity_" . $current_entity_id . "_values cv where cv.items_id=e.id and cv.fields_id='" . $field['id'] . "' and cv.value in (select id from app_global_lists_choices fc where fc.lists_id='" . $cfg->get('use_global_list') . "' and name like '%" . str_replace(['and','or'],' ',implode('',$search_keywords)) . "%'))>0";
                }
                else
                {                       
                   $sql_query[] = "(select count(*) as total from app_entity_" . $current_entity_id . "_values cv where  cv.items_id=e.id and cv.fields_id='" . $field['id'] . "' and cv.value in (select id from app_fields_choices fc where fc.fields_id='" . $field['id'] . "' and name like '%" . str_replace(['and','or'],' ',implode('',$search_keywords)) . "%'))>0";
                }
            }
